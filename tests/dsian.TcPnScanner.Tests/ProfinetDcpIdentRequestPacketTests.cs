...
public class ProfinetDcpIdentRequestPacketTests : VerifyBase
{
    const string PCAP_FILE = "test-data/pn-dcp.ident-req.pcap";

    public ProfinetDcpIdentRequestPacketTests()
        : base()
    {
    }
    ...
    public async Task ShouldSuccessfullyParseAllPackets()
    {
        ...
        {
            Assert.True(ProfinetDcpIdentRequestPacket.TryParse(packet, out var actual));
